    ...
    public function send(Request $request, $friendId)
{
    ...
    // Meriksa hubungan mutual
    $isMutual = Wishlist::where('user_id', $authUser->id)
        ->where('wishlist_user_id', $friend->id)
        ->exists() &&
        Wishlist::where('user_id', $friend->id)
        ->where('wishlist_user_id', $authUser->id)
        ->exists();

    if (!$isMutual) {
        return redirect()->route('home')->with('error', 'You cannot send a message to this user.');
    }
    ...
    return redirect()->route('chat.start', ['friendId' => $friend->id])->with('message', 'Message sent!');
}
